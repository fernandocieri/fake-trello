...
export default function Myboards() {
  ...
  useEffect(() => {
    async function getBoards() {
      let response = await axios.get(
        `https://api.trello.com/1/organizations/${organization.id}/boards?key=${credentials.key}&token=${credentials.token}`
      );
  ...
  async function createBoard(newBoardName) {
    ...
    const newBoard = await axios.post(
      `https://api.trello.com/1/boards/?name=${newBoardName}&key=${credentials.key}&token=${credentials.token}`
    );
